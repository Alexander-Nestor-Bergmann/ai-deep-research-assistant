...
from ai_deep_research_assistant.graph.workflow import run_research
...
class ResearchInterface:
    ...
    def _initialize_workflow(self):
        ...
        try:
            ...
            if self.enable_debug:
                self.console.print(
                    f"[{CLIConfig.MUTED_COLOR}]Research workflow initialized for session: {self.thread_id}[/]"
                )
        except Exception as e:
            self.console.print(
                f"[{CLIConfig.ERROR_COLOR}]Error initializing workflow: {e}[/]"
            )
        ...
        try:
            ...
            if os.path.exists(CLIConfig.HISTORY_FILE):
                with open(CLIConfig.HISTORY_FILE, "r") as f:
                    ...
                    self.query_history = self.query_history[
                        -CLIConfig.MAX_HISTORY_ITEMS :
                    ]
        except Exception as e:
            if self.enable_debug:
                self.console.print(
                    f"[{CLIConfig.MUTED_COLOR}]Could not load history: {e}[/]"
                )
    ...
    def _save_history(self):
        ...
        try:
            ...
            if self.enable_debug:
                self.console.print(
                    f"[{CLIConfig.MUTED_COLOR}]Could not save history: {e}[/]"
                )
    ...
    def display_history(self):
        ...
        if not self.query_history:
            self.console.print(
                f"[{CLIConfig.MUTED_COLOR}]No query history available.[/]"
            )
            return

        history_table = Table(
            title="Recent Query History", show_header=True, header_style="bold cyan"
        )
    ...
    def get_user_input(self) -> Optional[str]:
        ...
        try:
            ...
            query = Prompt.ask(
                f"[{CLIConfig.PRIMARY_COLOR}]🔍 Research Query[/]", console=self.console
            ).strip()
    ...
    async def execute_research_with_progress(self, query: str) -> Dict[str, Any]:
        ...
        with Progress(
            ...
            try:
                ...
                if result.get("final_synthesis"):
                    ...
                    formatted_result = {
                        ...
                        "error_details": result.get(
                            "error_message", "Unknown workflow issue"
                        ),
                    }
                ...
                execution_time = time.time() - start_time
                progress.update(
                    main_task, description=f"❌ Research failed: {str(e)[:50]}"
                )
    ...
    def _display_citations(self, sources: List):
        ...
        for i, source in enumerate(sources[:10], 1):  # Limit to top 10 sources
            ...
            citations_table.add_row(str(i), title, url, relevance_text)
        ...
        while True:
            try:
                ...
                if query is None:
                    # User wants to quit
                    self.console.print(
                        f"[{CLIConfig.SUCCESS_COLOR}]Thank you for using the Research Assistant! 👋[/]"
                    )
                    break
...
def create_cli_interface(
    enable_debug: bool = False, quick_mode: bool = False
) -> ResearchInterface:
    """
