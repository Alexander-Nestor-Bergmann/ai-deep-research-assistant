...
try:
    from ..state.schemas import Citation, ResearchFinding, ResearchDepth, ConfidenceLevel
    ...
    from state.schemas import Citation, ResearchFinding
...
class FormatterConfig:
    ...
    SECTION_HIERARCHY = ["Executive Summary", "Key Findings", "Detailed Analysis", "Evidence", "Conclusions", "Limitations"]
...
class CitationFormatter:
    ...
    def format_citation(self, citation: Union[Citation, Dict[str, Any]]) -> str:
        ...
        try:
            ...
            if self.style == "APA":
                ...
                return self._format_chicago(title, url, domain, published_date, source_info)
            elif self.style == "IEEE":
                return self._format_ieee(title, url, domain, published_date, source_info)
            else:
                return self._format_apa(title, url, domain, published_date, source_info)
    ...
    def format_bibliography(self, citations: List[Union[Citation, Dict[str, Any]]]) -> str:
        """
        ...
        try:
            ...
            sorted_citations = sorted(
                citations, 
                key=lambda c: (c.get('title', '') if isinstance(c, dict) else getattr(c, 'title', ''))
            )
    ...
    def _format_apa(self, title: str, url: str, domain: str, date: str, source_info: Dict) -> str:
    ...
    def _format_mla(self, title: str, url: str, domain: str, date: str, source_info: Dict) -> str:
    ...
    def _format_chicago(self, title: str, url: str, domain: str, date: str, source_info: Dict) -> str:
    ...
    def _format_ieee(self, title: str, url: str, domain: str, date: str, source_info: Dict) -> str:
    ...
    def _analyze_source(self, url: str, domain: str, title: str) -> Dict[str, Any]:
        ...
        # Academic sources
        if any(indicator in url_lower for indicator in ['.edu', 'arxiv', 'pubmed', 'scholar', 'jstor']):
        ...
        # News sources
        elif any(news in domain_lower for news in ['news', 'cnn', 'bbc', 'reuters', 'ap.org']):
    ...
    def _parse_date(self, date_str: str, mla_format: bool = False, ieee_format: bool = False) -> str:
        ...
        try:
            ...
            if "ago" in date_str.lower():
                ...
                return datetime.now().strftime("%Y, %B %d" if not mla_format else "%d %B %Y")
...
class ConfidenceVisualizer:
    ...
    def create_confidence_indicator(self, confidence: float, include_percentage: bool = True) -> Text:
        """
    ...
    def create_confidence_panel(self, 
                              confidence: float, 
                              title: str = "Confidence Assessment",
                              include_description: bool = True) -> Panel:
        """
    ...
    def create_confidence_table(self, confidence_data: List[Dict[str, Any]]) -> Table:
        ...
        """
        table = Table(title="Confidence Analysis", show_header=True, header_style="bold cyan")
        ...
        for item_data in confidence_data:
            ...
            level_indicator = self.create_confidence_indicator(confidence, include_percentage=False)
    ...
    def _get_confidence_description(self, confidence: float) -> str:
        ...
        if confidence >= 0.9:
            return "Very high confidence - multiple reliable sources in strong agreement"
        elif confidence >= 0.8:
...
class ResearchTraceFormatter:
    ...
    def format_research_trace(self, 
                            reasoning_steps: List[Dict[str, Any]], 
                            title: str = "Research Reasoning Trace") -> Panel:
        """
        ...
        trace_tree = Tree(f"[bold {FormatterConfig.PRIMARY_COLOR}]Research Process[/bold {FormatterConfig.PRIMARY_COLOR}]")
    ...
    def format_decision_points(self, decisions: List[Dict[str, Any]]) -> Table:
        ...
        """
        table = Table(title="Key Decision Points", show_header=True, header_style="bold magenta")
    ...
    def format_source_evaluation(self, evaluations: List[Dict[str, Any]]) -> Table:
        ...
        """
        table = Table(title="Source Quality Evaluation", show_header=True, header_style="bold blue")
...
class AnswerFormatter:
    ...
    def format_complete_answer(self,
                             query: str,
                             response: str,
                             findings: List[ResearchFinding],
                             citations: List[Citation],
                             confidence: float,
                             metadata: Optional[Dict[str, Any]] = None) -> Panel:
        """
        ...
        # Executive Summary
        executive_summary = self._create_executive_summary(query, response, confidence, metadata)
    ...
    def _create_executive_summary(self, query: str, response: str, confidence: float, metadata: Dict[str, Any] = None) -> str:
        ...
        if len(response) > FormatterConfig.MAX_ABSTRACT_LENGTH:
            summary = response[:FormatterConfig.MAX_ABSTRACT_LENGTH].rsplit('.', 1)[0] + "."
        else:
...
{bibliography}"""
    ...
    def _format_metadata_section(self, metadata: Dict[str, Any]) -> str:
        ...
        if metadata.get('execution_time'):
            metadata_lines.append(f"**Execution Time:** {metadata['execution_time']:.2f} seconds")
        
        if metadata.get('sources_analyzed'):
            metadata_lines.append(f"**Sources Analyzed:** {metadata['sources_analyzed']}")
