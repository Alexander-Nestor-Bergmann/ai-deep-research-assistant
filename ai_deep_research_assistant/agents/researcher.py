...
from ai_deep_research_assistant.tools.brave_tools import search_with_retry
from ai_deep_research_assistant.config.settings import get_settings
from ai_deep_research_assistant.agents.planner import AgentType
...
class ResearcherDeps(BaseModel):
    ...
    max_sources: int = Field(
        ...
        default_factory=lambda: get_settings().max_search_results,
    )
...
class ResearchOutput(BaseModel):
    ...
    confidence_score: float = Field(
        description="Overall confidence in the research (0.0-1.0)"
    )
...
async def web_search(
    ctx: RunContext[ResearcherDeps], query: str, count: int = 5
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Search failed: {str(e)}", "results": [], "query_used": query}
