...
class GuardrailDeps(BaseModel):
    ...
    conversation_context: Optional[str] = Field(description="Previous conversation context", default=None)


class GuardrailOutput(BaseModel):
    """Output from guardrail classification."""
    is_research_request: bool = Field(description="Whether this requires research or can be answered conversationally")
    ...
    suggested_research_type: Optional[str] = Field(description="Suggested research approach if applicable", default=None)
    estimated_sources_needed: int = Field(description="Estimated number of sources needed", default=1)
    can_answer_immediately: bool = Field(description="Whether this can be answered without web search")
    requires_current_info: bool = Field(description="Whether current/recent information is needed")
...
def should_route_to_research(classification: GuardrailOutput, confidence_threshold: Optional[float] = None) -> bool:
    ...
    return classification.confidence >= confidence_threshold or classification.complexity_estimate in ["moderate", "complex"]
