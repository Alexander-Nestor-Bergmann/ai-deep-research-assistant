...
class Settings(BaseSettings):
    ...
    model_top_p: float = Field(
        ...
        ge=0.0,
        le=1.0, 
        description="Model top-p for nucleus sampling"
    )
    ...
    first_token_timeout: int = Field(
        ...
        ge=1,
        le=30,
        description="Timeout for first token in seconds"
    )
    ...
    max_citations_per_claim: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum citations per research claim"
    )
    
    default_research_depth: ResearchDepth = Field(
        default=ResearchDepth.COMPREHENSIVE,
    ...
    show_progress_bars: bool = Field(
        default=True,
        description="Show progress bars during execution"
    )
    
    show_citation_previews: bool = Field(
        default=True,
        description="Show citation previews in CLI"
    )
    ...
    mock_brave_api: bool = Field(
        default=False,
        description="Use mock Brave API for testing"
    )
    ...
    @field_validator('brave_api_key', 'openrouter_api_key', 'openai_api_key', 
                     'anthropic_api_key', 'gemini_api_key', mode='before')
    @classmethod
