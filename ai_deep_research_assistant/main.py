...
For more information, visit: https://github.com/Alexander-Nestor-Bergmann/ai-deep-research-assistant
    ...
    output_group.add_argument(
        "--save-session",
        help="Save complete session data to file"
    )
    output_group.add_argument(
        "--no-color",
        action="store_true",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {AppConfig.APP_VERSION}"
    )
...
class ApplicationMonitor:
    ...
    def record_query(self, query: str, execution_time: float, success: bool, details: Dict[str, Any] = None):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "queries_per_minute": (
                (self.metrics["queries_processed"] / uptime) * 60 
                if uptime > 0 else 0
            ),
            "success_rate": (
    ...
    def print_performance_report(self):
        ...
        if summary['recent_queries']:
            ...
            for i, query in enumerate(summary['recent_queries'], 1):
                ...
                print(f"   {i}. {status} {query['query']} ({query['execution_time']:.2f}s)")
...
async def run_interactive_mode(args: argparse.Namespace, monitor: ApplicationMonitor) -> int:
...
async def run_single_query_mode(args: argparse.Namespace, monitor: ApplicationMonitor) -> int:
    ...
    try:
        ...
        if result.get('final_synthesis'):
            ...
            formatted_result = {
                'query': args.query,
                'response': result.get('conversation_response', 'No response generated'),
                'sources': [],
                'confidence': 1.0 if result.get('conversation_response') else 0.0,
                'execution_time': execution_time,
                'workflow_success': success,
                'workflow_type': 'conversation' if result.get('skip_research') else 'error',
                'error_details': result.get('error_message')
            }
        
        # Record metrics
        monitor.record_query(args.query, execution_time, success, {
...
async def save_results(result: Dict[str, Any], output_path: str, format_type: str):
    ...
    try:
        ...
        if format_type == "json":
            ...
            markdown_content = format_as_markdown(result, "APA")  # Default citation style
            with open(output_file, 'w', encoding='utf-8') as f:
    ...
    try:
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.StreamHandler(sys.stdout) if args.verbose > 0 else logging.NullHandler()
            ]
        )
        
        logger.info(f"Application starting: {AppConfig.APP_NAME} v{AppConfig.APP_VERSION}")
