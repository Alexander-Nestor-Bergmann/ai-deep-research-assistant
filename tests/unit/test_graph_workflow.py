...
class TestGraphWorkflow:
    ...
    def mock_settings(self):
        ...
        with patch('graph.workflow.get_settings') as mock_get_settings:
            mock_settings = Mock()
    ...
    async def test_guardrail_node_research_classification(self, sample_research_state, mock_settings):
        """Test guardrail node with research query classification."""
        with patch('graph.workflow.classify_query') as mock_classify:
            with patch('graph.workflow.should_route_to_research') as mock_should_route:
    ...
    async def test_guardrail_node_conversation_classification(self, sample_conversational_state, mock_settings):
        """Test guardrail node with conversational query classification."""
        with patch('graph.workflow.classify_query') as mock_classify:
            with patch('graph.workflow.should_route_to_research') as mock_should_route:
                with patch('graph.workflow.handle_conversation') as mock_conversation:
                    # Mock classification for conversational query
                    mock_classification = GuardrailOutput(
                        is_research_request=False,
                        confidence=0.95,
                        reasoning="Simple greeting query",
                    ...
                    # mock_conversation_result = ConversationResponse(
                    #     response="Hello! How can I help you today?",
                    #     confidence=0.95,
                    #     reasoning="Friendly greeting response",
                    #     response_type="greeting",
                    #     suggested_follow_ups=["What would you like to know?"]
                    # )
                    # mock_conversation.return_value = mock_conversation_result

                    # result = await guardrail_node(sample_conversational_state)
    ...
    async def test_guardrail_node_error_handling(self, sample_research_state, mock_settings):
    ...
    async def test_planning_node_success(self, sample_research_state, mock_settings):
        ...
        with patch('graph.workflow.create_research_plan') as mock_plan:
    ...
    async def test_planning_node_error_handling(self, sample_research_state, mock_settings):
    ...
    async def test_research_node_parallel_execution(self, sample_research_state, mock_settings):
        ...
        with patch('graph.workflow.conduct_research') as mock_research:
    ...
    async def test_research_node_sequential_execution(self, sample_research_state, mock_settings):
    ...
    async def test_research_node_no_plan_error(self, sample_research_state, mock_settings):
    ...
    async def test_synthesis_node_success(self, sample_research_state, mock_settings):
        ...
        with patch('graph.workflow.synthesize_research') as mock_synthesize:
            ...
            assert result["final_synthesis"] == mock_synthesis
            assert result["error_message"] is None
            assert "completed_at" in result

    async def test_synthesis_node_missing_data_error(self, sample_research_state, mock_settings):
    ...
    @pytest.mark.parametrize("query,expected_workflow", [
        ("Hello", "conversation"),
    ...
    async def test_run_research_workflow_routing(self, query, expected_workflow, mock_settings):
        """Test end-to-end workflow routing."""
        with patch('graph.workflow.create_research_graph') as mock_create_graph:
    ...
    async def test_run_research_graph_integration(self, mock_settings):
        ...
        with patch('graph.workflow.create_research_graph') as mock_create_graph:
            ...
            # Verify integration
            assert result["final_synthesis"]["final_answer"] == "Integration test complete"
            assert "completed_at" in result
