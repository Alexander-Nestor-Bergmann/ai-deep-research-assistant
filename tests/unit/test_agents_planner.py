...
class TestPlannerAgent:
    ...
    def mock_settings(self):
        ...
        with patch(
            "ai_deep_research_assistant.agents.planner.get_settings"
    ...
    async def test_create_simple_research_plan(
        self, sample_simple_query, available_agents, mock_settings
    ):
    ...
    async def test_create_complex_research_plan(
        self, sample_complex_query, available_agents, mock_settings
    ):
        ...
        with patch.object(planner_agent, "run") as mock_run:
            ...
            assert (
                plan.parallel_execution is True
            )  # Complex queries should use parallel execution
    ...
    async def test_news_query_agent_prioritization(
        self, sample_current_events_query, mock_settings
    ):
        ...
        with patch.object(planner_agent, "run") as mock_run:
            ...
            mock_result.output = ResearchPlan(
                ...
                parallel_execution=True,
                success_criteria=[
                    "Current AI regulation developments",
    ...
    async def test_academic_query_agent_prioritization(
        self, sample_academic_query, mock_settings
    ):
        ...
        with patch.object(planner_agent, "run") as mock_run:
            ...
            mock_result.output = ResearchPlan(
                ...
                parallel_execution=False,
                success_criteria=[
                    "Academic sources on quantum error correction",
            ...
            academic_tasks = [
                t for t in plan.tasks if t.agent_type == "academic_research"
            ]
    ...
    async def test_research_plan_with_limited_agents(
        self, sample_simple_query, mock_settings
    ):
    ...
    async def test_create_research_plan_error_handling(
        self, sample_simple_query, available_agents, mock_settings
    ):
    ...
    @pytest.mark.parametrize(
        "query_type,expected_parallel",
        [
            ("simple query", False),
            (
                "complex multi-faceted research question requiring analysis across multiple domains",
                True,
            ),
            ("current news about AI", False),  # Simple current events
            (
                "compare AI policies across countries with economic and social impact analysis",
                True,
            ),  # Complex comparison
        ],
    )
    async def test_parallel_execution_decisions(
        self, query_type, expected_parallel, available_agents, mock_settings
    ):
    ...
    async def test_task_keyword_generation(
        self, sample_complex_query, available_agents, mock_settings
    ):
    ...
    async def test_success_criteria_generation(
        self, sample_complex_query, available_agents, mock_settings
    ):
        ...
        with patch.object(planner_agent, "run") as mock_run:
            ...
            assert all(
                isinstance(criterion, str) for criterion in plan.success_criteria
            )
            assert all(
                len(criterion.strip()) > 10 for criterion in plan.success_criteria
