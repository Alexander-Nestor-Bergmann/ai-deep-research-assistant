...
class TestConversationAgent:
    ...
    async def test_handle_capability_conversation(self, sample_capability_queries):
        ...
        with patch.object(conversation_agent, 'run') as mock_run:
            ...
            assert "capabilities" in response.response.lower() or "help" in response.response.lower()
    ...
    async def test_handle_creative_conversation(self, sample_creative_queries):
        ...
        with patch.object(conversation_agent, 'run') as mock_run:
            ...
            assert "space" in response.response.lower() or "star" in response.response.lower()
    ...
    async def test_handle_conversation_with_context(self):
        ...
        with patch.object(conversation_agent, 'run') as mock_run:
            ...
            assert "technical" in response.response or "mathematical" in response.response
    ...
    async def test_empty_query_handling(self):
        ...
        with patch.object(conversation_agent, 'run') as mock_run:
            ...
            assert "question" in response.response.lower() or "help" in response.response.lower()
            assert len(response.suggested_follow_ups) > 0
    ...
    async def test_confidence_scores_appropriate(self):
        ...
        test_cases = [
            ...
            ("Explain quantum mechanics", 0.85, "explanation"),  # Lower for complex topics
