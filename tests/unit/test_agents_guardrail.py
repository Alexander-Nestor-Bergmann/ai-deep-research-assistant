...
class TestGuardrailAgent:
    ...
    def mock_settings(self):
        ...
        with patch(
            "ai_deep_research_assistant.agents.guardrail.get_settings"
    ...
    async def test_classify_research_query(
        self, sample_research_queries, mock_settings
    ):
        ...
        with patch.object(guardrail_agent, "run") as mock_run:
            ...
            for query in sample_research_queries[
                :3
    ...
    async def test_classify_conversational_query(
        self, sample_conversational_queries, mock_settings
    ):
    ...
    def test_should_route_to_research_custom_threshold(self, mock_settings):
        ...
        assert (
            should_route_to_research(classification, confidence_threshold=0.5) is True
        )

        # With custom threshold of 0.8, should not route to research
        assert (
            should_route_to_research(classification, confidence_threshold=0.8) is False
        )
    ...
    @pytest.mark.parametrize(
        "complexity,should_research",
        [("simple", False), ("moderate", True), ("complex", True)],
    )
